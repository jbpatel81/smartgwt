package com.ey.devopstest.client;
...
import com.smartgwt.sample.showcase.client.SampleResultsManager;
import com.smartgwt.sample.showcase.client.ShowcaseConfiguration;
import com.smartgwt.sample.showcase.client.ShowcaseCustomTile;
import com.smartgwt.sample.showcase.client.ShowcaseMessages;
import com.smartgwt.sample.showcase.client.ShowcasePanel;
import com.smartgwt.sample.showcase.client.SideNavTree;
import com.smartgwt.sample.showcase.client.SourceEntity;
import com.smartgwt.sample.showcase.client.TileView;
...
public class Consumerbank implements EntryPoint, HistoryListener, ShowcaseNavigator {
	...
	public static native boolean websiteModeSetOnPage() /*-{
		return $wnd.isc_websiteMode === true
				|| ($wnd.isc.params.isc_websiteMode === "true");
	}-*/;
	...
	private final String BETA_MESSAGE = "<br><br><span style='color: red;font-size:11px;font-weight: 700;'>BETA</span> : "
	...
	public void onModuleLoad() {
		final boolean useDesktopMode = ShowcaseConfiguration.getSingleton().isOpenForTesting()
				|| Browser.getIsDesktop();
		...
		// If the request contains param "websiteMode", override the configured
		// value
		{
			final String isc_websiteModeParam = com.google.gwt.user.client.Window.Location
					.getParameter("isc_websiteMode");
		...
		if (useDesktopMode)
			splitPane.setDeviceMode(DeviceMode.DESKTOP);
		...
		// Use the name 'search' so that we get a Search button on iOS instead
		// of a Go button.
			...
			public String hoverHTML(Object value, ListGridRecord record, int rowNum, int colNum) {
				if (record instanceof ExplorerTreeNode) {
					...
					if (factory != null && factory.getDescription() != null) {
						String customDiv = "<div style=\"width:450px; " + "margin-top:10px; " + "margin-bottom:10px; "
								+ "margin-left:10px; " + "margin-right:10px;\">" + "<b>Sample Description</b>: "
								+ factory.getDescription();

						if (node.getHTML().contains("BETA")) {
							customDiv += "<br><br><span style='color: red;font-size:11px;font-weight: 700;'>BETA</span> : "
									+ "This sample demonstrates features available in the next available version of "
									+ "Smart GWT, " + preReleaseVersion + ".  To download a " + preReleaseVersion
		...
		final Label version = new Label(
				M.versionLabelContents(Version.getVersion(), "" + Version.getBuildDate()).asString());
		version.setWidth100();
			...
			mainTabSet.addTabSelectedHandler(new TabSelectedHandler() {
				public void onTabSelected(TabSelectedEvent event) {
					...
					if (pane instanceof ShowcasePanel) {
						...
						AutoTest.setTestRoot(panel.shouldWrapViewPanel() ? viewPanel.getParentElement() : viewPanel);
					}
		...
		tileView = new TileView(sideNav.getShowcaseData(), useDesktopMode, sideNav.hasBetaSamples(), this);
		...
		if (isc_websiteMode) {
			...
			Label smartGwtButtonR = new Label("<div style='font-size: 9pt;''>Prefer to write UI in JavaScript?</div>"
					...
					+ "style='height: 30pt; float: left; margin-right: 2pt; " + "margin-left: 2pt; margin-top: 10pt;'/>"
					+ "<a target='_top' style='line-height: 12pt; padding-top: 5pt;' " + "href='" + "/#Welcome" + "' "
			...
			Label contactUsButtonR = new Label(
					"<div>Got questions?" + "<div style='font-size: 8pt;'>We'd love to hear from you!</div>" + "</div>"
							+ "<a target='_top' href='" + "/company/contact.jsp" + "' " + ">" + "Contact Us" + "</a>");
			...
			Label freeTrialButtonR = new Label("<div>Try it out for yourself!</div>"
					+ "<a target='_top' style='line-height: 12pt; padding-top: 5pt;' " + "href='"
					+ "/product/download.jsp" + "' " + ">"
			...
			Label pricingTrialButtonR = new Label("<div>Want your own?</div>" + "<a target='_top' href='" + "/product/"
					+ "' " + ">" + "Editions & Pricing" + "</a>");
			...
			Label learnMoreButtonR = new Label("<div>Isomorphic has the advantage.</div>" + "<a target='_top' href='"
					+ "/technology/whysmart.jsp" + "' " + ">" + "Learn More >" + "</a>");
			...
			Label freeTrialButtonB = new Label("<a target='_top' href='/product/download.jsp' " + ">Free Trial</a>");
			...
			Label pricingButtonB = new Label("<a target='_top' href='/product/' " + ">Editions & Pricing</a>");
			...
			Label smartGWTButtonB = new Label(
					"<a target='_top' style='width: 130pt;' " + "href='/#Welcome'" + " >SmartClient Live Demo</a>");
		...
		navTitleLabelProperties.addIconClickHandler(new IconClickHandler() {
			...
			public void onIconClick(IconClickEvent event) {
				com.google.gwt.user.client.Window.open("https://github.com/isomorphic-software/smartgwt", "sgwt", null);
			}
		});
		...
		if (useDesktopMode) {
			...
			selectItemDensity.setPrompt("Most skins support the ability to choose different data density "
					...
					+ "productivity gains in some applications when more data " + "is viewable at once");
		...
		// if (useDesktopMode)
		// printButton.setTitle(M.printButtonTitle().asString());
		...
		if (useDesktopMode)
			sourceButton.setTitle(M.viewSourceButtonTitle().asString());
		...
		if (useDesktopMode) {
			...
			splitPane.addPaneChangedHandler(new PaneChangedHandler() {
				@Override
				public void onPaneChanged(PaneChangedEvent event) {
					// Disable the search field whenever we're not displaying
					// the 'navigation'
					// pane so that the user cannot use the Prev button on the
					// virtual keyboard
		...
		if (SC.hasFirebug() && useDesktopMode) {
			...
			label.setContents(
					"Firebug can make the Showcase run slowly. For the best performance, we suggest disabling Firebug on this site.");
	...
	private void enableDetailTools() {
		...
		if (splitPane.getDeviceMode() == DeviceMode.DESKTOP) {
			// TabSet.tabBarControls is not writable.
			// mainTabSet.setTabBarControls(detailTools.toArray(new
			// Object[detailTools.size()]));
		} else {
			splitPane.setDetailToolButtons(detailTools.toArray(new Canvas[detailTools.size()]));
		}
	}
			...
			if (useDesktopMode) {
				...
				if (tab == null) {
					...
					tab.setTitle("<nobr>" + Canvas.imgHTML(icon, 16, 16) + "&nbsp;<span "
							+ "style='display:inline-block;line-height:16px;vertical-align:" + "text-top'>" + folderName
							+ "</span></nobr>");
			...
			if ("main".equals(explorerTreeNode.getNodeID())) {
				...
				if (useDesktopMode) {
					...
					if (tab == null) {
						...
						// store history token on tab so that when an already
						// open is selected,
						...
						tab.setTitle("<nobr>" + Canvas.imgHTML(icon, 16, 16) + "&nbsp;"
								+ "<span style='display:inline-block;line-height:16px;" + "vertical-align:text-top'>"
								+ sampleName + "</span></nobr>");
						tab.setPane(panel);
						tab.setCanClose(true);
						mainTabSet.addTab(tab);
					} else {
	...
	private Canvas createTableCanvas(FolderTreeNode folderTreeNode) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			ExplorerTreeNode node2 = (secondColIndex < children.length ? (ExplorerTreeNode) children[secondColIndex++]
					: null);
	...
	private void showNode(ExplorerTreeNode node, boolean saveLastOpened) {
		...
		TreeNode[] parents = (sideNav.getTree().getParents(node) != null) ? sideNav.getTree().getParents(node) : null;
		if (parents != null) {
		...
		// if the matched node is a folder, auto-expand it (probably want to see
		// what's inside)
		if (sideNav.getTree().isFolder(node) && !sideNav.getTree().isOpen(node)) {
			if (saveLastOpened)
				lastOpenedFolders.add(node);
			sideNav.getTree().openFolder(node);
		}
	}

	private void findNode() {
		if ((sideNav == null) || (sideNav.getData() == null))
			return;
		...
		for (int i = startIndex; i < des.length; i++) {
			...
			if (node.getName().toLowerCase().contains(search)) {
				...
				if (lastName.matches("/<.*>/")) {
					...
					newValue = searchRe.replace(node.getHTML(),
							"$1$2<span style='background-color:#00B2FA;'>$3</span>");
				} else {
					RegExp searchRe = RegExp.compile("(" + search + ")", "ig");
